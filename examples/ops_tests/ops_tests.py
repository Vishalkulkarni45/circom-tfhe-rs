...
fn main()  -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut out_add: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_add"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_add"][i], &client_key);
        out_add.push(val1 + val2);
    }

    let mut out_div: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_div"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_div"][i], &client_key);
        out_div.push(val1 / val2);
    }

    let mut out_eq: Vec<FheBool> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_eq"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_eq"][i], &client_key);
        out_eq.push(val1.eq(val2));
    }

    let mut out_gt: Vec<FheBool> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_gt"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_gt"][i], &client_key);
        out_gt.push(val1.gt(val2));
    }

    let mut out_geq: Vec<FheBool> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_geq"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_geq"][i], &client_key);
        out_geq.push(val1.ge(val2));
    }

    let mut out_lt: Vec<FheBool> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_lt"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_lt"][i], &client_key);
        out_lt.push(val1.lt(val2));
    }

    let mut out_leq: Vec<FheBool> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_leq"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_leq"][i], &client_key);
        out_leq.push(val1.le(val2));
    }

    let mut out_sub: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_sub"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_sub"][i], &client_key);
        out_sub.push(val1 - val2);
    }

    let mut out_mul: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_mul"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_mul"][i], &client_key);
        out_mul.push(val1 * val2);
    }

    let mut out_shl: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_shl"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_shl"][i], &client_key);
        out_shl.push(val1 << val2);
    }

    let mut out_shr: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_shr"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_shr"][i], &client_key);
    ...
    let mut out_xor: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_xor"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_xor"][i], &client_key);
        out_xor.push(val1^val2);
    }

    let mut out_mod: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_mod"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_mod"][i], &client_key);
        out_mod.push(val1 % val2);
    }

    let mut out_or: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_or"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_or"][i], &client_key);
        out_or.push(val1 | val2);
    }

    let mut out_and: Vec<FheUint64> = vec![];
    for i in 0..N {
        let val1 = FheUint64::encrypt(arrays["0.in1_and"][i], &client_key);
        let val2 = FheUint64::encrypt(arrays["0.in2_and"][i], &client_key);
    ...
    for i in 0..N {
        ...
        outputs.insert(str, out_eq[i].decrypt(&client_key) as u64);
    }

    for i in 0..N {
        let str: String = String::from("0.out_gt") + "[" + &i.to_string() + "]";
        outputs.insert(str, out_gt[i].decrypt(&client_key) as u64);
    }

    for i in 0..N {
        let str: String = String::from("0.out_geq") + "[" + &i.to_string() + "]";
        outputs.insert(str, out_geq[i].decrypt(&client_key) as u64);
    }

    for i in 0..N {
        let str: String = String::from("0.out_lt") + "[" + &i.to_string() + "]";
        outputs.insert(str, out_lt[i].decrypt(&client_key) as u64);
    }

    for i in 0..N {
        let str: String = String::from("0.out_leq") + "[" + &i.to_string() + "]";
        outputs.insert(str, out_leq[i].decrypt(&client_key) as u64);
    }
    ...
    for i in 0..N {
        ...
        outputs.insert(str, out_neq[i].decrypt(&client_key) as u64);
    }
